(() => {
  ...
  const apply = (d, fallback = {}) => {
    state = {
      items: Array.isArray(d?.items) ? d.items : (fallback.items || []),
      current_index: Number.isInteger(d?.current_index) ? d.current_index : (fallback.current_index ?? -1),
      mode: d?.mode || fallback.mode || "manual"
    };
    persist();
    return state;
  };
  ...
  async function playIndex(index) {
    ...
    const d = await setCurrent(index);
    const track = d.items[d.current_index];
    if (!track) return null;
    window.__dakshNowPlayingTrack = track;
    window.__dakshNowPlayingQueueIndex = d.current_index;
    await playTrackDirect(track, d.items, d.mode);
    return track;
  }
  ...
  async function playCurrent() {
    ...
    if (!Number.isInteger(index) || index < 0 || index >= state.items.length) index = 0;
  ...
  function sameQueue(items) {
    ...
    return items.every((x,i) => String(x.id ?? x.track_id) === String(state.items[i].id ?? state.items[i].track_id));
  }
  ...
  async function openQueue() {
    document.getElementById("dakshQueueOverlay")?.remove();
  ...
  async function syncCurrentFromTrack(track) {
    const id = track?.id ?? track?.track_id;
    if (id == null) return;
    const index = state.items.findIndex(x => String(x.id ?? x.track_id) === String(id));
  ...
  window.__dakshQueuePlayTrack = async (track, list, mode) => {
    if (!Array.isArray(list) || !list.length) {
      ...
      return playTrackDirect(track, state.items, state.mode);
    }
    return playTrackDirect(track, list, mode || "manual");
  };
