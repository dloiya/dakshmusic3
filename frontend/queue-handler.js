(() => {
  const API="/api/v1", QUEUE_API="/api/v1/playback-queue", QUEUE_KEY="daksh-queue-v5", cacheName="device-audio-v1";
  ...
  // The legacy app.js has its own lexical openQueue() which reads /playlist.
  // When Queue is selected, rewrite only that legacy request to the dedicated
  // playback queue endpoint. This keeps Playlist completely independent.
  const nativeFetch=window.fetch.bind(window);
  window.fetch=async function(input,init){
    try{
      const url=typeof input==="string"?input:input?.url||"";
      const home=document.querySelector("#list-home li.sel .name")?.textContent?.trim();
      const queueVisible=document.getElementById("view-queue")?.classList.contains("active")||document.getElementById("dakshQueueOverlay");
      if(home==="Queue" && queueVisible && /\/api\/v1\/playlist(?:[/?]|$)/.test(url)){
        const target=new URL(QUEUE_API,location.origin).toString();
  ...
  // Keep the queue's current index synchronized with whatever track the player
  // actually starts, so reopening Queue never jumps back to item 0.
  function syncCurrentFromTrack(track){
    const id=track?.id??track?.track_id;
    if(id==null)return;
    const idx=state.items.findIndex(x=>String(x.id??x.track_id)===String(id));
    if(idx>=0 && idx!==state.current_index) setCurrent(idx).catch(()=>{});
  }
  ...
  window.__setPlaylistSearchQueue=async tracks=>{const clean=Array.isArray(tracks)?tracks.filter(Boolean):[];if(!clean.length)return;await sync();if(state.items.length&&!confirm(`Replace the current queue with ${clean.length} search result${clean.length===1?"":"s"}?`))return;await replace(clean,0,"search");await playIndex(0);};
  window.__dakshQueuePlay=async track=>{await sync();const idx=state.items.findIndex(x=>String(x.id??x.track_id)===String(track?.id??track?.track_id));if(idx>=0)return playIndex(idx);showNowPlaying(track);};
  ...
  function homeQueueSelected(){return document.querySelector("#list-home li.sel .name")?.textContent?.trim()==="Queue";}
  function interceptQueueActivation(e){if(!homeQueueSelected())return;e.preventDefault();e.stopImmediatePropagation();openQueue();}
  document.addEventListener("click",e=>{const li=e.target.closest?.("#list-home li");if(li?.querySelector(".name")?.textContent?.trim()==="Queue")interceptQueueActivation(e);},true);
  document.addEventListener("click",e=>{if(e.target.closest?.("#btnCenter")&&homeQueueSelected())interceptQueueActivation(e);},true);
  document.addEventListener("keydown",e=>{if((e.key==="Enter"||e.key===" ")&&homeQueueSelected())interceptQueueActivation(e);},true);
