...
const naturalKey = (x, i) => x.source_id
  ? `apple-${slug(x.source_id)}`
  : `apple-${slug(x.title)}-${slug(x.artist)}-${slug(x.album)}-${Number(x.duration_ms) || 0}-${i}`;

async function seed(env, req) {
  ...
  for (let offset = 0; offset < items.length; offset += 150) {
    ...
    await env.DB.batch(chunk.map((x, i) => {
      ...
      const sourceUrl = x.source_url || (x.source_id
        ? `https://music.apple.com/us/song/${slug(x.title)}/${String(x.source_id)}`
        : null);
      return env.DB.prepare(`
        INSERT OR IGNORE INTO tracks
          (source,source_id,source_url,title,artist,album,duration_ms,isrc,artwork_url,natural_key,play_count)
        VALUES ('apple',?,?,?,?,?,?,?,?,?,0)
      `).bind(
        sourceId,
        sourceUrl,
        ...
        x.artist || "",
        x.album || null,
        Number(x.duration_ms) || null,
        x.isrc || null,
        x.artwork_url || null,
        naturalKey(x, offset + i),
      );
    }));
  }

  const { results: sourceRows = [] } = await env.DB.prepare(`
    SELECT id,source_id,title FROM tracks
    WHERE source='apple' AND (source_url IS NULL OR source_url='') AND source_id IS NOT NULL
  `).all();
  for (let offset = 0; offset < sourceRows.length; offset += 100) {
    const chunk = sourceRows.slice(offset, offset + 100);
    await env.DB.batch(chunk.map(t => env.DB.prepare(`
      UPDATE tracks SET source_url=? WHERE id=?
    `).bind(
      `https://music.apple.com/us/song/${slug(t.title)}/${String(t.source_id).replace(/^apple:/, "")}`,
      t.id,
    )));
  }
  ...
  for (let offset = 0; offset < items.length; offset += 100) {
    ...
    const { results = [] } = await env.DB.prepare(`
      SELECT id,source_id,title,artist,album,artwork_url,duration_ms
      FROM tracks WHERE source_id IN (${placeholders})
    `).bind(...ids).all();
  ...
  const byKey = new Map(playlistRows.map(t => [
    t.source_id || `title:${t.title}|artist:${t.artist}|album:${t.album || ""}`,
    t,
  ]));
  const ordered = [];
  for (let i = 0; i < items.length; i++) {
    const x = items[i];
    const key = x.source_id
  ...
  for (let offset = 0; offset < ordered.length; offset += 150) {
    ...
    await env.DB.batch(chunk.map((t, i) => env.DB.prepare(`
      INSERT INTO playlist_entries(track_id,position,title,artist,album,artwork_url,duration_ms)
      VALUES(?,?,?,?,?,?,?)
    `).bind(t.id, offset + i + 1, t.title, t.artist, t.album, t.artwork_url, t.duration_ms)));
  }
  ...
  if (topTracks.length) {
    await env.DB.batch(topTracks.map((t, i) => env.DB.prepare(`
      INSERT INTO top_played_cache(rank,track_id,storage_key,updated_at)
  ...
  return json({
    ok: true,
    playlist_entries: ordered.length,
    cache_entries: topTracks.length,
    cache_limit: 200,
    top100: topTracks.map((t, i) => ({ rank: i + 1, track_id: t.id, title: t.title, artist: t.artist, play_count: 0 })),
    missing_count: items.length - ordered.length,
    metadata_missing_after_seed: missingCount,
    play_counts_reset: true,
    metadata_backfill: { deferred: true, batch_limit: 20, remaining: missingCount },
  });
}

export async function handleLibraryV3(req, env) {
  const url = new URL(req.url);
  if (url.pathname === "/api/v1/library/seed" && req.method === "POST") return seed(env, req);
  return null;
}
