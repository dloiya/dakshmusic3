...
function buildNaturalKey(title, artist, album, source, sourceId, durationMs) {
  ...
  const identity = sourceId
    ? `${slug(source || "source")}-${slug(sourceId)}`
    : `duration-${Math.max(0, Number(durationMs) || 0)}`;

  return `${base}-${identity}`;
}

async function findOrPrepareTrack(
  env,
  {
    source,
    source_id,
...
async function upsertLightAlbum(
  env,
  {
    source_id,
    title,
...
async function refreshTopPlayed(env) {
  ...
  const old = await env.DB.prepare(`
    SELECT track_id, storage_key
    FROM top_played_cache
  `).all();
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    const storage =
      cached?.drive_file_id ||
      oldKeys.get(Number(results[i].id)) ||
      null;

    await env.DB.prepare(`
      INSERT INTO top_played_cache(
        rank,
        track_id,
        storage_key,
        updated_at
...
/**
 * Resolve canonical duration for Apple Music imports.
 *
 * Apple imports can have source IDs such as:
 *
 *   apple-1794881221
 *   apple:1794881221
 *   1794881221
 *
 * iTunes lookup requires the numeric catalog ID.
 */
async function resolveAppleDuration(env, track, trackId) {
  if (
    String(track.source || "").toLowerCase() !== "apple"
  ) {
    return track;
  }
  ...
  const catalogId = rawSourceId.replace(
    /^apple[-_:]/i,
    ""
  );
  ...
  if (!/^\d+$/.test(catalogId)) {
    console.warn(
      "Unable to resolve Apple catalog ID from source_id",
      rawSourceId
    );
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        "Apple duration lookup returned HTTP",
        response.status,
        catalogId
      );
    ...
    const song = (data.results || []).find(
      item =>
        ...
        Number(item.trackTimeMillis) > 0
    );

    if (!song?.trackTimeMillis) {
      console.warn(
        "Apple catalog returned no canonical duration",
        catalogId
      );
    ...
    console.log(
      "Resolved Apple canonical duration",
      JSON.stringify({
        track_id: trackId,
...
async function dispatchWarm(
  ...
  if (existing) {
    ...
    await env.DB.prepare(`
      ...
      SET
        status='failed',
        error='Timed out waiting for the acquisition workflow',
        updated_at=CURRENT_TIMESTAMP
      WHERE id=?
    `)
      .bind(existing.id)
  ...
  /*
   * Apple imports may not have duration_ms.
   *
   * Resolve the canonical Apple/iTunes catalog duration
   * before applying the strict identity guard.
   */
  if (!track.duration_ms) {
    track = await resolveAppleDuration(
      env,
      track,
      trackId
    );
  ...
  /*
   * Never bypass identity validation.
   *
   * If the canonical duration still cannot be resolved,
   * acquisition is refused.
   */
  if (!track.duration_ms) {
    throw new Error(
      `Track ${
        track.natural_key || trackId
      } has no canonical duration_ms; refusing acquisition without identity data`
    );
  }
  ...
  const response = await fetch(
    `https://api.github.com/repos/${env.GITHUB_OWNER}/${env.GITHUB_REPO}/actions/workflows/acquire-audio.yml/dispatches`,
    {
      method: "POST",
...
async function uploadAudio(
  ...
  if (
    ...
    return json(
      {
        error:
          `Job is not uploadable from status ${job.status}`,
      },
      409
    );
  }
  ...
  if (!claimed.meta?.changes) {
    return json(
      {
        error:
          "Job is no longer accepting uploads",
      },
      409
    );
  }
...
async function deezerSearch(
    ...
    (data.data || [])
      .map(x => {
        ...
        if (a === wantArtist) {
          ...
          a.includes(wantArtist) ||
          wantArtist.includes(a)
        ) {
          score += 1;
        }
  ...
  return scored[0]?.score >= 4
    ? scored[0].x
    : null;
}

async function appleImport(
  env,
  req,
  ctx
) {
  if (
    !(await requireAuth(env, req))
  ) {
    return json(
      {
        error:
  ...
  if (!items.length) {
    return json(
      {
        error:
          "No Apple Music tracks supplied",
      },
      400
    );
  }
      ...
      const track =
        await findOrPrepareTrack(
          env,
          {
            ...
            artwork_url:
              found.album?.cover_xl ||
              found.album?.cover_big ||
              null,
          }
        );
    ...
    (async () => {
      for (const id of missing) {
        try {
          ...
          console.error(
            "Top-100 acquisition dispatch failed",
            t?.natural_key || id,
            e
          );
        }
  ...
  try {
    warm =
      await warmWorkingCache(
        env,
        ctx
      );
  } catch (e) {
    console.error(
      "Post-import warm failed",
      e
    );
  }
...
async function addPlaylistEntry(
  ...
  if (body.album_id) {
    try {
      ...
      console.error(
        "Album upsert failed (playlist add)",
        body.album_id,
        e
      );
    }
  ...
  const pos =
    await env.DB.prepare(`
      SELECT
        COALESCE(MAX(position),0)+1 AS p
      FROM playlist_entries
    `).first();
  ...
  try {
    ...
    console.error(
      "Playlist-add acquisition dispatch failed",
      track.natural_key ||
        track.id,
      e
    );
...
async function playlistMutation(
  env,
  req,
  entryId
) {
...
async function listStoredAlbums(
  env,
  req
) {
  if (
...
async function cacheWholeAlbum(
  env,
  albumId,
  name,
  artist,
      ...
      for (
        ...
        if (!protectedRow) {
          try {
            ...
            console.error(
              "Album cache R2 eviction failed",
              t.drive_file_id,
              e
            );
          }
    ...
    trackIds.map(
      async trackId => {
        ...
        try {
          ...
          console.error(
            "Album cache acquisition dispatch failed",
            t?.natural_key ||
              trackId,
            e
          );
...
async function albumDetail(
  ...
  try {
    await upsertRichAlbum(
      env,
      data
    );
  } catch (e) {
    console.error(
      "Rich album upsert failed",
      albumId,
      e
    );
  }
  ...
  try {
    ...
    if (!jobId) {
      return json(
        {
          error:
            "Track has no source URL to acquire from",
        },
        400
      );
    }
...
async function clearAllData(
  ...
  for (
    ...
    try {
      ...
      console.error(
        "Failed to collect R2 keys from",
        table,
        e
      );
    }
  ...
  for (
    ...
    try {
      ...
      console.error(
        "Failed to delete R2 object during clear-all",
        key,
        e
      );
    }
...
async function callback(
  ...
  if (!result.meta?.changes) {
    return json(
      {
        error:
          "Job state changed before callback was accepted",
      },
      409
    );
  }
