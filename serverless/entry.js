...
function cookie(req) { const value = req.headers.get("Cookie") || ""; return value.match(/(?:^|;\s*)music_session=([^;]+)/)?.[1] || null; }
...
async function requireAuth(env, req) {
  const token = cookie(req); if (!token || !env.DB) return false;
...
async function refreshTopPlayed(env) {
  ...
  const oldRows = await env.DB.prepare(`SELECT track_id, storage_key FROM top_played_cache`).all();
  const oldKeys = new Map((oldRows.results || []).map(r => [Number(r.track_id), r.storage_key]));
...
async function dispatchTopWarm(env, trackId) {
  const active = await env.DB.prepare(`SELECT id FROM download_jobs WHERE track_id=? AND kind='general' AND status IN ('queued','dispatched','running') ORDER BY created_at DESC LIMIT 1`).bind(trackId).first();
  ...
  if (!response.ok) { const text=await response.text(); await env.DB.prepare(`UPDATE download_jobs SET status='failed',error=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(text,id).run(); throw new Error(`GitHub dispatch failed: ${response.status} ${text}`); }
...
async function restoreTopIntoWorking(env, trackId, storageKey) {
  if (!storageKey || !env.AUDIO_BUCKET) return false;
  const object = await env.AUDIO_BUCKET.head(storageKey);
  if (!object) return false;
  await env.DB.prepare(`INSERT OR REPLACE INTO general_cache(track_id,drive_file_id,last_accessed_at) VALUES(?,?,CURRENT_TIMESTAMP)`).bind(trackId,storageKey).run();
  return true;
}
...
async function warmWorkingCache(env, ctx) {
  ...
  const immediate=candidates.slice(0,5), backload=candidates.slice(5,25);

  const warm = async id => {
    const topRow = await env.DB.prepare(`SELECT storage_key FROM top_played_cache WHERE track_id=?`).bind(id).first();
    if (await restoreTopIntoWorking(env,id,topRow?.storage_key)) return;
    try { await dispatchTopWarm(env,id); } catch(e) { console.error("Cache warm failed",id,e); }
  };

  for (const id of immediate) await warm(id);
  if (backload.length) ctx.waitUntil((async()=>{ for (const id of backload) await warm(id); })());
...
async function uploadAudio(env, req, jobId) {
  ...
  const url=new URL(req.url), provider=url.searchParams.get("provider")||null, format=url.searchParams.get("format")||"flac";
  const contentType=req.headers.get("content-type")||(format==="mp3"?"audio/mpeg":"audio/flac"), storageKey=`tracks/${job.track_id}.${format}`;
  ...
  if (job.kind==='general') {
    ...
    for(const row of old.results||[]) {
      ...
      const top=await env.DB.prepare(`SELECT 1 FROM top_played_cache WHERE track_id=?`).bind(row.track_id).first();
      if (!top && row.drive_file_id) { try { await env.AUDIO_BUCKET.delete(row.drive_file_id); } catch(e) { console.error("R2 eviction failed",row.drive_file_id,e); } }
    }
  }
...
async function addPlaylistEntry(env, body) {
  ...
  if(!track){
    const result=await env.DB.prepare(`INSERT INTO tracks(source,source_id,source_url,title,artist,album,album_id,duration_ms,artwork_url) VALUES(?,?,?,?,?,?,?,?,?)`).bind(body.source||"deezer",body.source_id||null,body.source_url||null,body.title,body.artist||null,body.album||null,body.album_id||null,body.duration_ms||null,body.artwork_url||null).run();
...
export default {
  async fetch(req,env,ctx){
    ...
    if(url.pathname==='/api/v1/playlist'&&req.method==='POST'){
      ...
      if(authCheck.status===401||!authCheck.ok) return authCheck;
      let body; try{body=await req.clone().json();}catch{return json({error:"Invalid JSON body"},400);}
    ...
    if(url.pathname==='/api/v1/cache/top'&&req.method==='GET'){
      ...
      const tracks=await refreshTopPlayed(env); return json({count:tracks.length,items:tracks});
    }
