...
async def auth_session(session: str | None = Cookie(default=None), settings: Settings = Depends(get_settings), db: Store = Depends(store)):
    if not session:
...
@router.get("/status")
async def status(s: Store = Depends(store)):
...
async def login(payload: dict[str, Any], response: Request, settings: Settings = Depends(get_settings), s: Store = Depends(store)):
    password = str(payload.get("password", ""))
    if not settings.admin_password or not hmac.compare_digest(password, settings.admin_password):
        raise HTTPException(401, "Invalid credentials")
    token = secrets.token_urlsafe(32); now=int(time.time()); digest=_hash(token)
    await s.db.execute("INSERT OR REPLACE INTO sessions(id_hash,created_at,expires_at,last_seen_at) VALUES(?,?,?,?)", [digest,now,now+60*60*24*30,now])
    from fastapi.responses import JSONResponse
    out=JSONResponse({"ok":True}); out.set_cookie("session",token,max_age=60*60*24*30,httponly=True,samesite="lax",secure=settings.environment=="production"); return out
...
async def session_info(session: str | None = Cookie(default=None), s: Store = Depends(store)):
    if not session: return {"ok":False,"authenticated":False}
...
async def update_track(track_id:int,payload:dict[str,Any],s:Store=Depends(store),_:str=Depends(auth_session)):
    ...
    if fields:
        await s.db.execute(f"UPDATE tracks SET {','.join(f'{f}=?' for f in fields)},updated_at=CURRENT_TIMESTAMP WHERE id=?",[payload[f] for f in fields]+[track_id])
...
async def delete_track(track_id:int,s:Store=Depends(store),_:str=Depends(auth_session)):
    await s.delete_track(track_id); return {"ok":True}
...
async def retry_job(job_id:str,s:Store=Depends(store),settings:Settings=Depends(get_settings),_:str=Depends(auth_session)):
    ...
    if not rows: raise HTTPException(404,"Acquisition job not found")
    await s.update_job(job_id,"cancelled"); new_id,_=await s.create_job(rows[0]["track_id"]); await GitHubActionsConnector(settings).dispatch(settings.acquire_workflow,{"track_id":str(rows[0]["track_id"]),"job_id":new_id}); await s.update_job(new_id,"dispatched"); return {"ok":True,"job_id":new_id}


@router.post("/seed")
async def seed(file:UploadFile=File(...),s:Store=Depends(store),_:str=Depends(auth_session)):
    content=await file.read(); return await s.import_csv(file.filename or "library.csv",content)
...
async def worker_callback(payload:dict[str,Any],s:Store=Depends(store)):
    # Workers authenticate through a shared callback secret in production.
...
async def playback(track_id:int,s:Store=Depends(store),_:str=Depends(auth_session)):
    ...
    return StreamingResponse(obj.body,media_type="audio/flac",headers={"Accept-Ranges":"bytes"})
