...
def store(settings:Settings=Depends(get_settings))->Store:return Store(settings)
...
async def auth_session(session:str|None=Cookie(default=None),db:Store=Depends(store)):
    ...
    await db.db.execute("UPDATE sessions SET last_seen_at=? WHERE id_hash=?",[int(time.time()),_hash(session)]);return session
@router.get("/status")
...
async def health(settings:Settings=Depends(get_settings),s:Store=Depends(store)):
    try: rows=await s.db.query("SELECT 1 AS ok");return {"ok":bool(rows and rows[0]["ok"]==1),"service":settings.app_name,"version":"2.0.0","database":True}
...
async def login(payload:dict[str,Any],settings:Settings=Depends(get_settings),s:Store=Depends(store)):
    if not settings.admin_password or not hmac.compare_digest(str(payload.get("password","")),settings.admin_password):raise HTTPException(401,"Invalid credentials")
    token=secrets.token_urlsafe(32);now=int(time.time());await s.db.execute("INSERT OR REPLACE INTO sessions(id_hash,created_at,expires_at,last_seen_at) VALUES(?,?,?,?)",[_hash(token),now,now+2592000,now])
...
async def session_info(session:str|None=Cookie(default=None),s:Store=Depends(store)):
    ...
    rows=await s.db.query("SELECT expires_at FROM sessions WHERE id_hash=?",[_hash(session)]);return {"ok":True,"authenticated":bool(rows and rows[0]["expires_at"]>int(time.time()))}
...
async def search(q:str=Query(min_length=1),limit:int=25,source:str="deezer",settings:Settings=Depends(get_settings),_:str=Depends(auth_session)):
    if source=="deezer":items=await DeezerConnector().search(q,limit)
    elif source=="spotiflac":items=await SpotiFlacConnector(settings).search(q,limit)
    elif source=="ytflac":items=await YtFlacConnector().search(q,limit)
    else:raise HTTPException(400,"Unsupported search source")
...
async def acquire(track_id:int,s:Store=Depends(store),settings:Settings=Depends(get_settings),_:str=Depends(auth_session)):
    ...
    if created:
        try:await GitHubActionsConnector(settings).dispatch(settings.acquire_workflow,{"track_id":str(track_id),"job_id":job_id,"source":str(track.get("source") or ""),"source_id":str(track.get("source_id") or ""),"source_url":str(track.get("source_url") or "")});await s.update_job(job_id,"dispatched")
        except Exception as exc:await s.update_job(job_id,"failed",str(exc));raise HTTPException(502,f"Acquire worker dispatch failed: {exc}")
...
async def retry_job(job_id:str,s:Store=Depends(store),settings:Settings=Depends(get_settings),_:str=Depends(auth_session)):
    ...
    if not rows:raise HTTPException(404,"Acquisition job not found")
    await s.update_job(job_id,"cancelled");track=await s.track(rows[0]["track_id"]);new_id,_=await s.create_job(track["id"]);await GitHubActionsConnector(settings).dispatch(settings.acquire_workflow,{"track_id":str(track["id"]),"job_id":new_id,"source":str(track.get("source") or ""),"source_id":str(track.get("source_id") or ""),"source_url":str(track.get("source_url") or "")});await s.update_job(new_id,"dispatched");return {"ok":True,"job_id":new_id}
@router.post("/seed")
...
async def playback(track_id:int,s:Store=Depends(store),_:str=Depends(auth_session)):
    ...
    await s.db.execute("UPDATE tracks SET play_count=play_count+1,updated_at=CURRENT_TIMESTAMP WHERE id=?",[track_id]);body=getattr(obj,"body",None) if not isinstance(obj,dict) else obj.get("Body")
