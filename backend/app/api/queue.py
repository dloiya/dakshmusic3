...
@router.get("")
async def get_queue(key:str="main",service:QueueService=Depends(get_queue)):
    return {"ok":True,**await service.get(key)}

@router.post("/shuffle")
async def shuffle_queue(key:str="main",service:QueueService=Depends(get_queue)):
    return {"ok":True,**await service.shuffle(key)}

@router.patch("/state")
async def update_queue_state(payload:dict[str,Any],key:str="main",service:QueueService=Depends(get_queue)):
    return {"ok":True,**await service.update_state(key,payload)}

@router.post("/{track_id}")
async def add_queue(track_id:int,key:str="main",position:int|None=None,service:QueueService=Depends(get_queue)):
...
async def remove_queue(entry_id:int,key:str="main",service:QueueService=Depends(get_queue)):
    await service.remove(key,entry_id); return {"ok":True}

@router.delete("")
async def clear_queue(key:str="main",service:QueueService=Depends(get_queue)):
