...
@router.get("")
async def get_playlist(service:PlaylistService=Depends(get_playlist)):
    return {"ok":True,"items":await service.get()}

@router.post("/tracks/{track_id}")
async def add_track(track_id:int,position:int|None=None,service:PlaylistService=Depends(get_playlist)):
    try: pos=await service.add(track_id,position)
    except ValueError as exc: raise HTTPException(404,str(exc))
    return {"ok":True,"track_id":track_id,"position":pos}

@router.delete("/{entry_id}")
async def remove_track(entry_id:int,service:PlaylistService=Depends(get_playlist)):
    await service.remove(entry_id); return {"ok":True}

@router.delete("")
async def clear_playlist(service:PlaylistService=Depends(get_playlist)):
