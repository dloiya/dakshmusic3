...
@router.get("/tracks")
async def list_tracks(q: str | None = None, limit: int = 100, offset: int = 0, service: LibraryService = Depends(get_library), _: str = Depends(require_session)):
    return {"ok": True, "items": await service.list_tracks(q, limit, offset)}

@router.get("/tracks/{track_id}")
async def get_track(track_id: int, service: LibraryService = Depends(get_library), _: str = Depends(require_session)):
    item = await service.get_track(track_id)
    if not item: raise HTTPException(404, "Track not found")
    return {"ok": True, "item": item}

@router.post("/tracks")
async def create_track(payload: dict[str, Any], service: LibraryService = Depends(get_library), _: str = Depends(require_session)):
    try: return {"ok": True, "item": await service.create_track(payload)}
    except ValueError as exc: raise HTTPException(422, str(exc))

@router.patch("/tracks/{track_id}")
async def update_track(track_id: int, payload: dict[str, Any], service: LibraryService = Depends(get_library), _: str = Depends(require_session)):
    try: return {"ok": True, "item": await service.update_track(track_id, payload)}
    except ValueError as exc: raise HTTPException(404, str(exc))

@router.delete("/tracks/{track_id}")
async def delete_track(track_id: int, service: LibraryService = Depends(get_library), _: str = Depends(require_session)):
    try: await service.delete_track(track_id)
    except ValueError as exc: raise HTTPException(404, str(exc))
    return {"ok": True}

@router.get("/albums")
async def list_albums(q: str | None = None, limit: int = 100, offset: int = 0, db: D1Repository = Depends(get_db), _: str = Depends(require_session)):
    return {"ok": True, "items": await LibraryRepository(db).albums(q, limit, offset)}

@router.get("/albums/{album_id}")
async def get_album(album_id: int, db: D1Repository = Depends(get_db), _: str = Depends(require_session)):
...
@router.get("/export")
async def export_library(db: D1Repository = Depends(get_db), _: str = Depends(require_session)):
...
async def playback(track_id: int, settings: Settings = Depends(get_settings), db: D1Repository = Depends(get_db), _: str = Depends(require_session)):
