...
class DataService:
    ...
    @classmethod
    def _track(cls, row):
        # This is the actual CSV schema: Track name, Artist name, Album,
        # Playlist name, Type, ISRC, Apple - id, 100 Cache.
    ...
    async def _metadata(self, track):
        # Prefer an exact ISRC query when possible, then fall back to title + artist.
        queries = []
        if track["isrc"]:
        ...
        for query in queries:
            try:
                ...
                if self._score(best, track) < 0.55:
                    continue
    ...
    async def _enrich_rows(self, tracks):
        # Keep concurrency bounded so a 100-row chunk doesn't overwhelm the
        # external provider or the Worker runtime.
    ...
    async def _insert_import_rows(self, job_id, rows):
        ...
        statements = []
        for track in tracks:
            # First update an existing exact match. This also repairs old rows
            # that were previously imported as Unknown metadata.
            statements.append((
                """UPDATE tracks SET
