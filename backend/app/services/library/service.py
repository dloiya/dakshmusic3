...
class LibraryService:
    ...
    async def list_tracks(self, q=None, limit=100, offset=0):
        return await self.store.tracks(q, limit, offset)

    async def get_track(self, track_id: int):
        return await self.store.track(track_id)
    ...
    async def create_track(self, data: dict):
        ...
        return await self.store.upsert_track(data)
    ...
    async def delete_track(self, track_id: int):
        if not await self.store.track(track_id):
            raise ValueError("Track not found")
        await self.store.delete_track(track_id)
