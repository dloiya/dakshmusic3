...
function corsHeaders(request){const origin=request.headers.get("Origin");return{"access-control-allow-origin":origin||"*","access-control-allow-methods":"GET,POST,DELETE,OPTIONS","access-control-allow-headers":"Content-Type, Authorization","access-control-max-age":"86400",vary:"Origin"};}
...
async function callRetriever(env,jobId,track,priority="normal"){
  ...
  const headers={Authorization:`Bearer ${env.OCI_API_TOKEN}`,Accept:"application/json","Content-Type":"application/json"};
  const upstream=await fetch(`${ociBase(env)}/acquire/${encoded}`,{method:"POST",headers,body:JSON.stringify({job_id:jobId,track_id:Number(track.id),source_url:url,title:track.title,artist:track.artist,album_name:track.album_name||null,priority})});
  const text=await upstream.text();
  let data={};
  try{data=JSON.parse(text);}catch{data={detail:text};}
  return {ok:upstream.ok,status:upstream.status,data};
}
async function ensureAcquisition(env,trackId,priority="normal",ctx=null){
  const track=await env.DB.prepare(`SELECT id,title,artist,album_name,source_url,storage_key,storage_status FROM tracks WHERE id=?`).bind(Number(trackId)).first();
  if(!track)return null;
  if(track.storage_status==="ready"&&track.storage_key)return {ready:true,track_id:Number(track.id)};
  const active=await env.DB.prepare(`SELECT id,status FROM acquisition_jobs WHERE track_id=? AND status IN ('queued','running') ORDER BY created_at DESC LIMIT 1`).bind(Number(trackId)).first();
  if(active)return {job_id:active.id,status:active.status,duplicate:true};
  const jid=crypto.randomUUID(),t=now();
  await env.DB.prepare(`INSERT INTO acquisition_jobs(id,track_id,status,worker,attempts,error,created_at,updated_at,started_at) VALUES(?,?, 'queued','oci',0,NULL,?,?,NULL)`).bind(jid,Number(trackId),t,t).run();
  const run=async()=>{
    try{
      await env.DB.prepare(`UPDATE acquisition_jobs SET status='running',worker='oci',attempts=attempts+1,started_at=?,updated_at=?,error=NULL WHERE id=?`).bind(now(),now(),jid).run();
      const result=await callRetriever(env,jid,track,priority);
...
async function playlistRows(db){const r=await db.prepare(`SELECT p.id AS playlist_entry_id,p.position,t.id,t.title,t.artist,t.album_id,t.album_name,t.source,t.source_id,t.source_url,t.isrc,t.duration_ms,t.artwork_url,t.storage_key,t.storage_status,t.play_count,t.metadata_json FROM playlist_entries p JOIN tracks t ON t.id=p.track_id ORDER BY p.position,p.id`).all();return r.results||[];}
async function queueRows(db,key){const r=await db.prepare(`SELECT q.id AS queue_entry_id,q.position,t.id,t.title,t.artist,t.album_id,t.album_name,t.source,t.source_id,t.source_url,t.artwork_url,t.duration_ms,t.storage_key,t.storage_status,t.play_count,t.metadata_json FROM queue_entries q JOIN tracks t ON t.id=q.track_id WHERE q.queue_key=? ORDER BY q.position`).bind(key).all();return r.results||[];}
async function setMode(db,mode){const t=now();await db.prepare(`INSERT INTO queue_state(queue_key,current_index,mode,shuffle_enabled,updated_at) VALUES('default',0,?,1,?) ON CONFLICT(queue_key) DO UPDATE SET current_index=0,mode=excluded.mode,updated_at=excluded.updated_at`).bind(mode,t).run();}
async function clearKey(db,key){await db.prepare(`DELETE FROM queue_entries WHERE queue_key=?`).bind(key).run();}
function csvEscape(v){const s=v==null?"":String(v);return /[",\n\r]/.test(s)?`"${s.replace(/"/g,'""')}"`:s;}
function toCsv(rows){const c=["id","title","artist","album_id","album_name","source","source_id","source_url","isrc","duration_ms","artwork_url","storage_key","storage_status","play_count","cache_requested","created_at","updated_at"];return[c.join(","),...rows.map(r=>c.map(k=>csvEscape(r[k])).join(","))].join("\n");}
function parseCsv(text){const lines=String(text||"").replace(/^\uFEFF/,"").split(/\r?\n/).filter(Boolean);if(!lines.length)return[];const parse=s=>{const a=[];let x="",q=false;for(let i=0;i<s.length;i++){const c=s[i];if(c==='"'){if(q&&s[i+1]==='"'){x+='"';i++;}else q=!q;}else if(c===','&&!q){a.push(x);x="";}else x+=c;}a.push(x);return a;};const h=parse(lines[0]);return lines.slice(1).map(l=>{const v=parse(l),o={};h.forEach((k,i)=>o[k]=v[i]??"");return o;});}
export async function watchdog(env,reason="scheduled"){if(env.OCI_API_URL&&env.OCI_API_TOKEN){const c=new AbortController(),timer=setTimeout(()=>c.abort(),5000);try{const r=await fetch(`${ociBase(env)}/health`,{headers:{Authorization:`Bearer ${env.OCI_API_TOKEN}`,Accept:"application/json"},signal:c.signal,cache:"no-store"});if(r.ok)return{status:"healthy",action:"none",reason};}catch{}finally{clearTimeout(timer);}}const instance=await getInstance(env),state=instance?.lifecycleState;if(state==="STOPPED"){await instanceAction(env,"START");return{status:"recovering",action:"START",lifecycleState:state,reason};}if(state==="STARTING"||state==="STOPPING")return{status:"waiting",action:"none",lifecycleState:state,reason};if(state==="RUNNING"){await instanceAction(env,"RESET");return{status:"recovering",action:"RESET",lifecycleState:state,reason};}return{status:"waiting",action:"none",lifecycleState:state,reason};}
async function handle(request,env,ctx){const url=new URL(request.url),path=url.pathname.replace(/\/+$/g,"")||"/",method=request.method.toUpperCase();if(method==="OPTIONS")return new Response(null,{status:204,headers:corsHeaders(request)});try{
...
if(path==="/api/library/tracks"&&method==="GET"){const n=Math.min(Math.max(Number(url.searchParams.get("limit")||500),1),2000);return json({tracks:await trackRows(env.DB,n)},200,request);}
if(path==="/api/library/albums"&&method==="GET"){const n=Math.min(Math.max(Number(url.searchParams.get("limit")||500),1),1000);return json({albums:await albumList(env.DB,n)},200,request);}
