...
function pemToDer(pem) {
  const body = String(pem).replace(/^\uFEFF/, "")
    ...
    .replace(/-----BEGIN RSA PRIVATE KEY-----/g, "").replace(/-----END RSA PRIVATE KEY-----/g, "")
    .replace(/\r?\nOCI_API_KEY\s*$/i, "").replace(/\s+/g, "");
...
async function signRequest({ method, url, body, tenancy, user, fingerprint, privateKeyPem }) {
  ...
  const headers = { "x-date": xDate, host: u.host };
  let signingString;
  let signedHeaders;

  if (normalizedMethod === "get" || normalizedMethod === "delete") {
    signedHeaders = "(request-target) x-date host";
    signingString = [`(request-target): ${target}`, `x-date: ${xDate}`, `host: ${u.host}`].join("\n");
  } else {
    const bodyText = body || "";
    ...
    signingString = [
      `(request-target): ${target}`, `x-date: ${xDate}`, `host: ${u.host}`,
      `content-length: ${contentLength}`, `content-type: application/json`, `x-content-sha256: ${contentSha256}`,
    ].join("\n");
  }

  const key = await importPrivateKey(privateKeyPem);
  const signature = await crypto.subtle.sign({ name: "RSASSA-PKCS1-v1_5" }, key, encoder.encode(signingString));
  headers.authorization = [
    'Signature version="1"', `keyId="${tenancy}/${user}/${fingerprint}"`, 'algorithm="rsa-sha256"',
    `headers="${signedHeaders}"`, `signature="${b64(new Uint8Array(signature))}"`,
  ].join(",");
  return headers;
}

function requireConfig(env) {
  const required = ["OCI_TENANCY_OCID", "OCI_USER_OCID", "OCI_KEY_FINGERPRINT", "OCI_PRIVATE_KEY", "OCI_INSTANCE_OCID", "OCI_REGION"];
...
export async function ociSignedRequest(env, method, path, body = "") {
  ...
  const url = `${base}${path}`;
  const headers = await signRequest({ method, url, body, tenancy: env.OCI_TENANCY_OCID, user: env.OCI_USER_OCID, fingerprint: env.OCI_KEY_FINGERPRINT, privateKeyPem: env.OCI_PRIVATE_KEY });
  ...
  if (!response.ok) throw new Error(`OCI API ${method} ${path} returned ${response.status}: ${text.slice(0, 500)}`);
  return data;
}

export async function getInstance(env) {
  return ociSignedRequest(env, "GET", `/20160918/instances/${encodeURIComponent(env.OCI_INSTANCE_OCID)}`);
}
...
export async function instanceAction(env, action) {
  return ociSignedRequest(
    env,
    "POST",
    `/20160918/instances/${encodeURIComponent(env.OCI_INSTANCE_OCID)}/actions/${encodeURIComponent(action)}`,
    ""
  );
}
