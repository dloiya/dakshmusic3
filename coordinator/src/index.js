...
function sourceFromUrl(raw) {
  try {
    ...
    if (h.includes('music.amazon.')) return 'amazon-music';
    if (h === 'music.youtube.com' || h === 'youtube.com' || h === 'youtu.be') return 'youtube';
    if (h === 'soundcloud.com') return 'soundcloud';
    if (h === 'bandcamp.com') return 'bandcamp';
    if (h === 'qobuz.com') return 'qobuz';
    if (h === 'deezer.com') return 'deezer';
...
function sourceIdFromUrl(raw) {
  try {
    const u = new URL(raw);
    const parts = u.pathname.split('/').filter(Boolean);
    const trackIndex = parts.findIndex(x => x.toLowerCase() === 'track');
    if (trackIndex >= 0 && parts[trackIndex + 1]) return parts[trackIndex + 1];
...
export default {
  async fetch(req, env) {
    ...
    if (req.method === 'POST' && p === '/api/jobs') {
      ...
      let track = null;
      const suppliedUrl = typeof b.url === 'string' ? b.url.trim() : '';

      if (b.track_id != null) {
        track = await env.DB.prepare(
          'SELECT id,title,artist,album_name,source,source_id,source_url,isrc,storage_key,storage_status FROM tracks WHERE id=?1'
        ).bind(Number(b.track_id)).first();
        if (!track) return err('track not found', 404);
      } else if (suppliedUrl) {
        track = await env.DB.prepare(
          'SELECT id,title,artist,album_name,source,source_id,source_url,isrc,storage_key,storage_status FROM tracks WHERE source_url=?1 LIMIT 1'
        ).bind(suppliedUrl).first();

        // The website accepts new source URLs, so create a minimal track record when
        // the URL is not already present. The acquisition worker resolves real metadata.
        if (!track) {
          const source = sourceFromUrl(suppliedUrl);
          const sourceId = sourceIdFromUrl(suppliedUrl);
          const inserted = await env.DB.prepare(
            `INSERT INTO tracks (title,artist,source,source_id,source_url,created_at,updated_at)
             VALUES (?1,?2,?3,?4,?5,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`
          ).bind('Pending acquisition', 'Unknown', source, sourceId, suppliedUrl).run();

          const newId = inserted.meta?.last_row_id;
          if (newId == null) return err('failed to create track record', 500);
          track = await env.DB.prepare(
            'SELECT id,title,artist,album_name,source,source_id,source_url,isrc,storage_key,storage_status FROM tracks WHERE id=?1'
          ).bind(newId).first();
        }
      } else {
        return err('track_id or url is required');
      }

      if (!track) return err('track could not be resolved', 500);
      if (track.storage_status === 'complete' && track.storage_key) {
        return json({ status: 'complete', track_id: track.id, already_acquired: true });
      }

      const existing = await env.DB.prepare(
        "SELECT id,status,attempts FROM acquisition_jobs WHERE track_id=?1 AND status IN ('queued','dispatched','running') ORDER BY created_at DESC LIMIT 1"
      ).bind(track.id).first();
      if (existing) return json({ id: existing.id, track_id: track.id, status: existing.status, attempts: existing.attempts });

      const id = crypto.randomUUID();
      await env.DB.prepare(
        "INSERT INTO acquisition_jobs(id,track_id,status,worker,attempts,created_at,updated_at) VALUES(?1,?2,'queued',NULL,0,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)"
      ).bind(id, track.id).run();
    ...
    if (req.method === 'POST' && p === '/api/jobs/claim') {
      ...
      const candidate = await env.DB.prepare(
        "SELECT id FROM acquisition_jobs WHERE status IN ('queued','dispatched') ORDER BY created_at LIMIT 1"
      ).first();
      if (!candidate) return json({ job: null });
      const changed = await env.DB.prepare(
        "UPDATE acquisition_jobs SET status=?1,worker=?2,attempts=attempts+1,started_at=COALESCE(started_at,CURRENT_TIMESTAMP),updated_at=CURRENT_TIMESTAMP WHERE id=?3 AND status IN ('queued','dispatched')"
      ).bind(RUNNING, browser, candidate.id).run();
      if (!changed.meta?.changes) return json({ job: null });
      const row = await env.DB.prepare(
        "SELECT j.id,j.track_id,j.attempts,t.title,t.artist,t.album_name,t.source,t.source_id,t.source_url,t.isrc FROM acquisition_jobs j JOIN tracks t ON t.id=j.track_id WHERE j.id=?1 AND j.status=?2 AND j.worker=?3"
      ).bind(candidate.id, RUNNING, browser).first();
    ...
    if (req.method === 'POST' && /^\/api\/jobs\/[^/]+\/upload$/.test(p)) {
      ...
      const url = new URL(`${env.R2_ENDPOINT.replace(/\/$/, '')}/${env.R2_BUCKET || 'dakshmusic3-audio'}/${key}`);
    ...
    if (req.method === 'GET' && p === '/api/jobs') {
      return json({ jobs: (await env.DB.prepare('SELECT j.*,t.title,t.artist,t.album_name,t.source,t.source_url,t.storage_key,t.storage_status FROM acquisition_jobs j JOIN tracks t ON t.id=j.track_id ORDER BY j.created_at DESC LIMIT 100').all()).results });
    }
